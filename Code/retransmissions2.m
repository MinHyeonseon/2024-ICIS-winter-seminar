...
for i = 1: test_time
    ...
    while true
        ...
        if M==4
            ...
            recovered_data(data_index) = QPSK_demapper(received_symbol(data_index)); %QPSK로 디매핑
        ...
        elseif M==16
            tmp = modulated_symbol(data_index)*h(h_index)* sqrt(transmit_power)+noise(h_index);
            received_symbol(data_index) = tmp/h(h_index); % 페이딩 데이터 복구
            recovered_data(data_index) = QAM16_demapper(received_symbol(data_index), transmit_power); %16QAM으로 디매핑
            % 받은 값과 보낸 값이 다르다면, 재전송 요구, 채널값 +1
            if recovered_data(data_index) ~= data(data_index)
                h_index = h_index+1;
                count = count+1;
    ...
    count_average(i) = sum(coount_time);
end
